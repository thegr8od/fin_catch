...
export const CustomAlert: React.FC<CustomAlertProps> = ({ message, onClose, onConfirm }) => {
  const handleConfirm = () => {
    onClose();
    if (onConfirm) {
      onConfirm();
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-[400px] transform transition-all">
        <div className="flex flex-col items-center">
          ...
          <div className="w-full">
            <button onClick={handleConfirm} className="w-full px-6 py-3 bg-blue-500 text-white rounded-lg font-korean-pixel hover:bg-blue-600 transition-colors">
              확인
            </button>
          </div>
        </div>
