...
const AiQuizPage = () => {
  ...
  // AI 퀴즈 훅 사용
  const { loading, getLatestQuizContent, submitQuizAnswer } = useAiQuiz();
  ...
  const fetchQuizzes = useCallback(async () => {
    setIsLoading(true);
    try {
      // 타입 문제 해결을 위해 명시적으로 타입 지정
      const response = await getLatestQuizContent() as unknown as QuizResponse;
      console.log("퀴즈 데이터 응답:", response);
      
      if (response && response.isSuccess && response.result) {
        // result가 QuizItem[]인지 확인하고 설정
        if (Array.isArray(response.result)) {
          ...
          console.error("퀴즈 데이터 형식이 올바르지 않습니다.");
        }
      } else {
  ...
  const handleSubmitAnswer = useCallback(async () => {
    ...
    try {
      // API로 정답 제출 - 타입 문제 해결을 위해 타입 캐스팅
      const response = await submitQuizAnswer(currentQuiz.quizId, selectedOption) as unknown as SubmitAnswerResponse;
      ...
      // 응답 확인 및 처리
      const isCorrect = response && response.isSuccess && isTrueValue(response.result);
      ...
      const isCorrect = selectedOptionData && selectedOptionData.isCorrect === true;
  ...
  const handleTimeUp = useCallback(async () => {
    ...
    try {
      // 시간 초과 시 임의의 오답 인덱스 찾기 (0을 기본값으로)
          ...
          <div className="w-[50%] mx-auto bg-white p-5 mt-4 border-2 border-primary border-opacity-40 rounded-2xl shadow-xl max-h-[72vh] overflow-y-auto lg:w-[60%] md:w-[70%] sm:w-[85%]">
            <div className="flex flex-col items-center">
              ...
              <div className="w-full bg-blue-50 p-4 rounded-xl mb-6">
                ...
                <p className="text-lg font-medium">정답률: <span className="font-bold text-blue-600">{quizzes.length > 0 ? Math.round((correctAnswers / quizzes.length) * 100) : 0}%</span></p>
