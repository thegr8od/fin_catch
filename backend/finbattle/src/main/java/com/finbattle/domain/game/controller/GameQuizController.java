...
public class GameQuizController {
    ...
    public void checkAnswer(@DestinationVariable String roomId, @Payload AnswerRequest request) {
        log.info("Received quiz answer for roomId: {} with answer: {} from user: {}",
                roomId, request.getUserAnswer(), request.getUserId());
        quizService.checkQuizAnswer(roomId, request.getUserAnswer(), request.getUserId());
    }
}
